(function($){
	$.widget("ui.queueManager",{
			...
			$('ul.queue',self.element).sortable({
				...
				'receive': function(event,ui){
					$.ajax({
						...
						data: 'action=changeOwner&task='+ui.item.attr('id').slice(2)+'&user='+self.options.userID
					});
				}
			}).disableSelection();
			$('#menuBtn')
		...
		_processItem: function(item){
			...
			if(item.completed == 'F'){
				html = inst._addTask(item.id, item);
				if(item.active == 'T'){
					active = true;
					html.addClass('queue-item-active');
					$('.stopwatch',html).button('option',{label:'Pause',icons:{primary: 'ui-icon-pause'}});
					$('#fancyClock').tzineClock('destroy').tzineClock({stopwatch: item.start_time,variance:item.daily_time});
				}
				html.appendTo(el).show('slide',{direction:'down',easing:'easeOutBounce'},500);
			}
		...
		_addTask: function(id, item){
			var template = '<li class="queue-item ui-corner-all" id="QI{{id}}" style=""><div class="queue-item-name editable">{{name}}</div><div class="queue-item-type editable">{{type}}</div><div class="details ui-corner-all top-level" style="bottom: auto; top: 0px; display: none; "><div class="details-header">Details</div><div class="details-notes editable details-notes-scroll">{{details}}</div><a class="queue-complete">Complete Task</a></div><a class="stopwatch">Start</a></li>',
				data = item || { id: id, name: "New Item", type: "--None--", details: "" };
				html = Mustache.to_html(template, data);
				result = $(html);
			
			result.data('variance',item.daily_time);
			$('.stopwatch',result).button({
				text:true,
			...
			$('.queue-complete',result).button({text: true,icons: {primary: 'ui-icon-check'}});
			return result;
		},
		...
		_delegation: function(){
			...
			$('a.queue-plus',el).bind('mousedown',function(){
				$.ajax({
					...
					success: function(data){
						var licount = data,
							html = inst._addTask(licount);					
						html.appendTo(ul).show('slide',{direction:'down',easing:'easeOutBounce'},500);;
						if(inst.options.table){
							inst._addTimesheetRow(licount);
						}
					}
				});
				return false;
			...
			el.delegate('.btnSave','mousedown', function(){
				var editable = $(this).parent(),
					...
					queueID = $(this).closest('li').attr('id').slice(2),
					dataObj = {
						'task': queueID,
			...
			el.delegate('.queue-complete','mousedown',function(){
				...
				$.ajax({
					type: 'POST',
					url: inst.options.url,
					data: 'action=complete&task='+lineitem.attr('id').slice(2),
					success: function(){
					...
					if(!disabled){
						...
						if($this.text() == "Start"){
							...
							$.ajax({
								type: 'POST',
								url: inst.options.url,
								data: 'action=start&task='+lineitem.attr('id').slice(2)
							});
							...
							var taskID = lineitem.attr('id').slice(2);
							$.ajax({
								type: 'POST',
								url: inst.options.url,
								data: 'action=pause&task='+lineitem.attr('id').slice(2),
								dataType: 'json',
								success: function(data){
							...
							success: function(data){
								if(!data){
									...
									$('<li>',{
										...
										.prependTo('.completes');
									$.each(data, function(i,item){
										var template = '<li class="complete-item ui-corner-all" id="UC{{id}}"><div class="complete-item-name">{{name}}</div><div class="complete-item-type">{{type}}</div><a class="complete-restore">Uncomplete</a></li>',
				...
				// BUTTONS: restores a completed task to the queue
					.delegate('.complete-restore','mousedown',function(){
						...
						$.ajax({
							type: 'POST',
							url: inst.options.url,
							data: 'action=unComplete&task='+lineitem.attr('id').slice(2),
							dataType: 'json',
							success: function(data){
								if(data.type == "") data.type = "--None--";
								ul.append(inst._addTask(data.id, data));
								$('li:last',ul)
	...
	var menu = $('<ul class="menu ui-corner-all">')
		...
		.append('<li><a id="btn-reports" href="#">Reports</a></li>')
	...
	$('#btn-reports').button({icons: {primary: 'ui-icon-print'}});
